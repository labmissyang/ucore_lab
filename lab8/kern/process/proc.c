...
load_icode(int fd, int argc, char **kargv) {
    ...
    struct elfhdr *elf = (struct elfhdr *)kmalloc(sizeof(struct elfhdr));
    if(elf==NULL)
      goto bad_pgdir_cleanup_mm;
    ...
    //(3.4) find every program section headers
        if(load_icode_read(fd,ph,sizeof(struct proghdr),elf->e_phoff+t*sizeof(struct proghdr))!=0){
            kfree(elf);
            kfree(ph);
            goto bad_pgdir_cleanup_mm;
        }
    ...
    uint32_t arg_size=0,i;
    for(i=0;i<argc;i++){
      arg_size+=strnlen(kargv[i],EXEC_MAX_ARG_LEN+1)+1;
    }

    char *stack_top=USTACKTOP-arg_size;
    char **argv=stack_top-argc*sizeof(uintptr_t);
    arg_size=0;
    for(i=0;i<argc;i++){
        argv[i]=strcpy((stack_top+arg_size),kargv[i]);
        arg_size+=strnlen(kargv[i],EXEC_MAX_ARG_LEN+1)+1;
    }
    stack_top=(char *)argv-sizeof(int);
    *((int *)stack_top)=argc;
